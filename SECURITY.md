...
- **iOS:** the same material lives in the Keychain as `kSecClassGenericPassword` items marked `ThisDeviceOnly` (never synced to iCloud): direct-mode tokens under one service keyed by origin, the server catalog under another, and Relay pairings under a third (`relay-targets`), so removing or renaming one side can never touch the other's keys. No `keychain-access-groups` entitlement is declared, so app extensions cannot read the Keychain. The **Add to Mercury** share extension exchanges only staged attachment files with the app through the App Group container and holds no credentials.
