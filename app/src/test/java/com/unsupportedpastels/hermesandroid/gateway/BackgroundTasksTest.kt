...
class BackgroundTasksTest {
    @Test fun duplicateReplayDoesNotRegressProgressOrRefreshActivityAge() {
        ...
        assertEquals(state, state.reduce(next.copy(sessionId = RuntimeSessionId("rebound")), RuntimeSessionId("rebound"), 10000))
    ...
    @Test fun replayedChildCompletionAfterOwnedRuntimeReplacementUpdatesOneRow() {
        ...
        val next = RuntimeSessionId("replacement")
        ...
        assertEquals(old, old.reduce(complete, RuntimeSessionId("foreign"), 2000))
    }
    ...
    @Test fun failuresStopsAndGlobalRegistryAbsenceAreNotSuccess() {
        ...
        val global = com.unsupportedpastels.hermesandroid.app.DelegationStatus(active = listOf(
            com.unsupportedpastels.hermesandroid.app.DelegatedSubagent("foreign", "Unknown ownership", "running"),
        ))
        assertEquals(stale, stale.reconcile(global, runtime))
        assertEquals(stale, stale.reconcile(com.unsupportedpastels.hermesandroid.app.DelegationStatus(), runtime))
    }
    ...
    @Test fun globalRegistryNeverReactivatesMissingIdentityFallback() {
        ...
        val status = com.unsupportedpastels.hermesandroid.app.DelegationStatus(active = listOf(
            com.unsupportedpastels.hermesandroid.app.DelegatedSubagent("identity-unavailable", "Not identified", "running"),
        ))
    ...
    @Test fun verifiedRuntimeRebindKeepsOnlyExactOwnedChildAndDoesNotRefreshAge() {
        val next = RuntimeSessionId("replacement")
        val old = BackgroundTasks().reduce(event("subagent.start"), runtime, 1000).unavailable()
        val registry = com.unsupportedpastels.hermesandroid.app.DelegationStatus(active = listOf(
            com.unsupportedpastels.hermesandroid.app.DelegatedSubagent("child", "Global description", "running"),
            com.unsupportedpastels.hermesandroid.app.DelegatedSubagent("foreign", "Other owner", "running"),
        ))
        ...
        assertEquals(old, old.reconcile(com.unsupportedpastels.hermesandroid.app.DelegationStatus(), next, previousRuntime = runtime))
    }

    @Test fun rebindRejectsAmbiguousRegistryAndKeepsTerminalAndForeignScopes() {
        val next = RuntimeSessionId("replacement")
        val old = BackgroundTasks().reduce(event("subagent.start"), runtime, 1000).unavailable()
        fun registry(vararg states: String) = com.unsupportedpastels.hermesandroid.app.DelegationStatus(active = states.map {
            com.unsupportedpastels.hermesandroid.app.DelegatedSubagent("child", "Global metadata", it)
        })
        ...
        assertEquals(old, old.reconcile(registry("running"), next, previousRuntime = RuntimeSessionId("foreign")))
    ...
    @Test fun terminalIsExactAndStaleRuntimeCannotContaminate() {
        val started = BackgroundTasks().reduce(event("subagent.start"), runtime, 1000)
        assertEquals(started, started.reduce(event("subagent.complete"), RuntimeSessionId("other"), 2000))
