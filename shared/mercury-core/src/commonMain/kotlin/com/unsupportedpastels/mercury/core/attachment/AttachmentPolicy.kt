...
/**
 * The policy-relevant view of a staged attachment. [dedupKey] identifies the
 * underlying source (Android: the content:// URI; iOS: the picked item's URL)
...
/**
 * Pure attachment policy shared by the Android and iOS clients: naming
 * hygiene, image-vs-file routing, byte/count caps (checked at metadata time
 * AND again while staging), and prompt-text assembly. Byte streaming stays in
 * each platform's I/O layer. Rejection and error strings are user-visible
 * contract: both platforms must surface them verbatim.
 */
